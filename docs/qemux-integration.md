...
| --- | --- | --- |
| Image size / leanness | Minimal: `debian:bookworm-slim` + `qemu-system-x86`, `qemu-utils`, Node, prod deps. | Heavier: also noVNC, websockets, passt, Python + `qemu.qmp`, qemux scripts — much of it unused if the entrypoint is overridden. |
